...
export interface KeyboardShortcut {
  ...
  exec: (
    ...
    cursors: Cursor[],
    event: KeyboardEvent
  ) => { code: string; cursors: Cursor[] } | undefined | null;
}

const keyboardShortcuts: KeyboardShortcut[] = [];
export const CodeEditor = ({ initialValue }) => {
  ...
  return (
    ...
    >
      <Code code={code} />
      <Cursors
        cursors={cursors.map((cursor) => validateCursor(code, cursor))}
      />
    </div>
  );
};
...
const Code: React.FC<{ code: string }> = ({ code }) => {
  ...
  useEffect(() => {
    setTokenizedCode(
      highlight(code, "typescript").map((v) => (
        <div className={styles.line} key={list_id++}>
          {v}
        </div>
      ))
    );
...
const DisplayCursor = ({ cursor }: { cursor: Cursor }) => {
  ...
  return (
    <div
      className={blink ? styles.cursor : styles.cursor_blink}
      style={{
        top: Char.height * cursor.line,
        left: Char.width * cursor.column,
      }}
    />
  );
};
